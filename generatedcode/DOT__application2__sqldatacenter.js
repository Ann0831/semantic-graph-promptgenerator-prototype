...
<END>
*/
/*  I modified testdatabase to newTestdatabase  */
const pool = mysql.createPool({
    host: 'testmysql-1.crseg8qams8s.ap-southeast-2.rds.amazonaws.com',
    user: sqlusername,
    password: sqlpassword,
...
function generatePostId(timestamp, userId) {
    ...
    const postIdStr = `${timestamp}${userIdStr}`;
    return BigInt(postIdStr);
}
...
function GlobalRecommendNumberCalculator(like_count, timeSincePosted) {
    return Math.round(
        1000 * like_count + 1000000 * Math.exp(-lambda * (timeSincePosted / 1000))
    );
}
...
async function checkuserexists(username) {
    try {
        const [rows] = await pool.query(
            'SELECT 1 FROM userinfo WHERE username = ? LIMIT 1',
            [username]
        );
...
async function searchusernamebyuserID(userID) {
    try {
        const [rows] = await pool.query(
            'SELECT username FROM userinfo WHERE user_id = ? LIMIT 1',
            [userID]
        );
        if (rows.length > 0) {
...
async function searchuserIDbyusername(username) {
    try {
        const [rows] = await pool.query(
            'SELECT user_id FROM userinfo WHERE username = ? LIMIT 1',
            [username]
        );
        if (rows.length > 0) {
...
async function searchlike_count(post_id) {
    ...
    try {
        const [rows] = await pool.query(
            'SELECT like_count FROM userpostsinfo WHERE post_id = ? LIMIT 1',
            [post_id.toString()]
        );
        if (rows.length > 0) {
...
async function unsendlike(user_id, post_id) {
    try {
        ...
        // Step 3: Update like_count in userpostsinfo
        const [updateResult] = await pool.query(
            `UPDATE userpostsinfo
             SET like_count = (
                 SELECT COUNT(*) FROM userlikesinfo WHERE post_id = ?
             )
             WHERE post_id = ?`,
            [post_id.toString(), post_id.toString()]
        );
...
async function readPostByPost_id(post_id) {
    try {
        const [rows] = await pool.query(
            'SELECT * FROM userpostsinfo WHERE post_id = ? LIMIT 1',
            [post_id.toString()]
        );
        if (rows.length > 0) {
            return rows[0];
...
Logic:
前往userLoginToken的對應username欄位，先看看他是不是null，如果是null就return false。看看資料庫的token和函式輸入的token是否相同。相同就返回true，不相同就返回false
...
async function Verify_LoginToken(username, token) {
    try {
        const [rows] = await pool.query(
            'SELECT token FROM userLoginToken WHERE username = ? LIMIT 1',
            [username]
        );

        if (rows.length === 0 || rows[0].token == null) {
            return false;
        }
...
async function Write_LoginToken(username) {
    try {
        ...
        const token = jwt.sign({ username, iat: issuedAt }, JWT_SECRET);

        await pool.query(
            `INSERT INTO userLoginToken (username, token, issued_at, created_at)
             VALUES (?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
             ON DUPLICATE KEY UPDATE token = VALUES(token), issued_at = CURRENT_TIMESTAMP, created_at = CURRENT_TIMESTAMP`,
            [username, token]
        );
...
<END>
*/
async function Write_HashedPassword(username, hashed_password) {
...
async function Verify_HashedPassword(username, hashedpassword) {
    try {
        const [rows] = await pool.query(
            'SELECT hashed_password FROM userHashedPassword WHERE username = ? LIMIT 1',
            [username]
        );
        if (rows.length === 0) {
            return false;
        }
        return rows[0].hashed_password === hashedpassword;
    } catch (err) {
        return false;
    }
}
...
async function Create_UserAccount(username, hashedpassword) {
    ...
    try {
        ...
        // Insert into userHashedPassword
        ...
        // Insert into userLoginToken with empty token initially
        await connection.query(
            `INSERT INTO userLoginToken (username, token, issued_at, created_at)
             VALUES (?, '', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`,
            [username]
...
async function updateAndSendGlobalRecommendNumber() {
    try {
        const [posts] = await pool.query(
            `SELECT post_id, like_count, secondTime 
             FROM userpostsinfo 
        ...
        for (const post of posts) {
            const timeSincePosted = now - Number(post.secondTime || 0);
            const score = GlobalRecommendNumberCalculator(
                post.like_count || 0,
                timeSincePosted
            );
            ...
            await pool.query(
                `UPDATE userpostsinfo 
                 SET globalrecommendscore = ? 
                 WHERE post_id = ?`,
                [score, post.post_id.toString()]
            );
        }

        const [result] = await pool.query(
            `SELECT post_id, globalrecommendscore, user_id 
             FROM userpostsinfo 
             WHERE is_deleted = FALSE`
        );
...
async function makepost(username, imagepath, article, secondTime) {
    try {
        ...
        const modifyInfoResult = await modifyuserpostsinfo(post_id, secondTime, article, imagepath);
        if (modifyInfoResult !== "success") return "fail";

        const modifyUserIDResult = await modifyuserpostsinfouserID(userID, post_id);
        if (modifyUserIDResult !== "success") return "fail";
...
現在要寫的元件的描述：
const sqldatacenter={// functions and gloalvariables that it own   //}
...
const sqldatacenter = {
  ...
  unsendlike,
  readPostByPost_id,
  updateAndSendGlobalRecommendNumber,
  searchlike_count,
  searchuserIDbyusername,
  searchusernamebyuserID,
  checkuserexists,
  Appenduserpostsinfo,
  generatePostId,
  modifyuserpostsinfo,
  modifyuserpostsinfouserID,
  makepost,
  diduserlikepost,
  Verify_LoginToken,
  Write_LoginToken,
  Write_HashedPassword,
  Verify_HashedPassword,
  Create_UserAccount,
  logout
};
