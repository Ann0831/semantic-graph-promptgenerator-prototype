...
<END>
*/
const returnMainpage = require('./returnMainpage');
...
logic: 先把req的cookie中的username和token抓出來，然後利用sqldatacenter.Verify_LoginToken來做確認。如果身分驗證正確，那就先用sqldatacenter.searchuserIDbyusername來查user_id，以及抓出req.body中的post_id來執行sqldatacenter.sendlike。如果一切成功，就res.send({"state":"success"})。如果過程中有任何失敗，例如身分驗證失敗，就res.send({"state":"fail"})。
...
const post_sendlike_handler = async (req, res) => {
  try {
    ...
    const verified = await sqldatacenter.Verify_LoginToken(username, token);
    if (!verified) {
      res.send({ state: "fail" });
      return;
    }
...
logic: 先把req的cookie中的username和token抓出來，然後利用sqldatacenter.Verify_LoginToken來做確認。如果身分驗證正確，那就先用sqldatacenter.searchuserIDbyusername來查user_id，以及抓出req.body中的post_id來執行sqldatacenter.unsendlike。如果一切成功，就res.send({"state":"success"})。如果過程中有任何失敗，例如身分驗證失敗，就res.send({"state":"fail"})。
...
const post_register_handler = async (req, res) => {
  try {
    ...
    const exists = await sqldatacenter.checkuserexists(requestedUsername);
    if (exists) {
...
const post_login_handler = async (req, res) => {
  try {
    ...
    const result = await sqldatacenter.Write_LoginToken(username);
...
/* ----------------prompt-----Id: filename:./application/index.js|name:get_userlikepost_handler|placeType:server|type:EndpointHandler---------------- */
/*
<START>

元件名稱：get_userlikepost_handler
類型：EndpointHandler
檔案：./application/index.js
你唯一可以使用的外部物件(包含函式)在這裡的第一層，更深的層的元件及其描述只是說明用(說明import自哪裡或是擁有甚麼)，不能用。注意喔，不要使用這邊以外的全域變數或是存取其他物件(ex.table)：
  - sqldatacenter (GlobalVariable) in ./application/index.js
...
檢查req中的cookie，如果有username和Token，就送去給sqldatacenter中的Verify_LoginToken驗證，如果成功就運用sqldatacenter.searchuserIDbyusername和username來找到user_id，以及在req.body找到post_id，用sqldatacenter.diduserlikepost及user_idp和post_id，如果回傳true則res.send("true")，其他情況則res.send("false");

請根據以上資訊生成此元件的功能程式碼(javascript)。就只生成這個元件喔，不要幫忙做export甚麼的，或是自己引入其他全域變數。這個軟體系統的預設背景是node.js、express、mysql

<END>
*/
const get_userlikepost_handler = async (req, res) => {
  try {
    const username = req.cookies.username;
    const token = req.cookies.token;
    ...
    if (user_id === null || post_id === undefined) {
      res.send("false");
      return;
    }

    const liked = await sqldatacenter.diduserlikepost(user_id, post_id);
    res.send(liked ? "true" : "false");
  } catch (err) {
    res.send("false");
  }
};
...
const post_makepost_handler = async (req, res) => {
  ...
  try {
    ...
    const imagePath = path.join(imageDir, `${post_id}${ext}`);
    fs.writeFileSync(imagePath, req.file.buffer);

    const article = req.body.article;
    const result = await sqldatacenter.makepost(username, imagePath, article, timestamp.toString());
...
/* ----------------prompt-----Id: filename:./application/index.js|name:/userlikepost|placeType:server|type:APIEndpoint[GET]---------------- */
/*
<START>
...
你唯一可以使用的外部物件(包含函式)在這裡的第一層，更深的層的元件及其描述只是說明用(說明import自哪裡或是擁有甚麼)，不能用。注意喔，不要使用這邊以外的全域變數或是存取其他物件(ex.table)：
  - app (GlobalVariable) in ./application/index.js
    描述：
         it is an external module. no need to write it
  - get_userlikepost_handler (EndpointHandler) in ./application/index.js
    描述：
         input:(req,res)
...
現在要寫的元件的描述：
app.get('/userlikepost',get_userlikepost_handler);
